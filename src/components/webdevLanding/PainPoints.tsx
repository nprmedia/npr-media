      ...
      <div className="mt-12 grid gap-8 md:grid-cols-3">
        {points.map((p, i) => (
          <motion.div
            ...
            className="rounded-2xl bg-[var(--color-antique)]/60 p-6 text-center shadow-xl ring-1 ring-[var(--color-charcoal)]/10 backdrop-blur-md hover:shadow-2xl"
          >
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--color-blood)]/20 text-[var(--color-blood)]">
              {p.icon}
            </div>
            <h3 className="mb-2 font-semibold text-[clamp(1.1rem,1.8vw,1.25rem)]">{p.title}</h3>
            <p className="text-sm text-[var(--color-charcoal)]">{p.text}</p>
          </motion.div>
        ))}
      </div>
    </section>
