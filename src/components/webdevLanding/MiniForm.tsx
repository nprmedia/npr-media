...
export default function MiniForm() {
  ...
  const baseInput = 'peer w-full rounded-md bg-[var(--color-antique)]/80 px-10 py-3 text-sm text-[var(--color-charcoal)] shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-blood)]/40 placeholder-transparent'
  const iconBase = 'absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--color-sepia)] pointer-events-none'
  const labelBase = 'pointer-events-none absolute left-10 top-1/2 -translate-y-1/2 text-sm text-[var(--color-sepia)] transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-focus:top-2 peer-focus:-translate-y-0 peer-focus:text-xs'
  ...
  return (
    <motion.form
      ...
      className="space-y-4 rounded-2xl bg-[var(--color-antique)]/20 p-6 shadow-xl ring-1 ring-[var(--color-charcoal)]/10 backdrop-blur"
    >
      ...
      <motion.button
        ...
        className="relative flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-blood)] py-3 font-semibold text-[var(--color-silver)] shadow-md transition hover:bg-[var(--color-crimson)]"
      >
      ...
      {success && <p className="pt-2 text-center text-sm text-[var(--color-silver)]">We’ll be in touch shortly.</p>}
    </motion.form>
  )
}
