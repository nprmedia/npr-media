...
export default function MiniForm() {
  ...
  const baseInput =
    'peer w-full rounded-md bg-offwhite px-10 py-3 text-sm text-charcoal shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blood/40 placeholder-transparent'
  const iconBase = 'pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-silver'
  const labelBase =
    'pointer-events-none absolute left-10 top-1/2 -translate-y-1/2 text-sm text-silver transition-all peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-focus:top-2 peer-focus:-translate-y-0 peer-focus:text-xs'
  ...
  return (
    <motion.form
      ...
      className="mx-auto box-border w-full max-w-md space-y-4 rounded-2xl bg-antique/20 p-6 shadow-xl ring-1 ring-silver/20 backdrop-blur"
    >
