        ...
        <div className="mt-12 grid gap-8 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="rounded-2xl bg-[var(--color-antique)]/70 p-6 shadow-xl ring-1 ring-[var(--color-charcoal)]/10 backdrop-blur-md hover:shadow-2xl"
            >
              <p className="text-sm italic">&ldquo;{t.quote}&rdquo;</p>
              <p className="mt-4 font-semibold">{t.name}</p>
              <p className="text-xs text-[var(--color-sepia)]">{t.title}</p>
            </motion.div>
          ))}
        </div>
      </div>
