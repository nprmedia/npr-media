  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 z-50 w-full transition-all ${
        scrolled ? 'bg-umber/90 shadow-md backdrop-blur-sm' : 'backdrop-blur-0 bg-transparent'
      } ${textColor}`}
    >
      <div
      ...
      >
        ...
        <nav className="hidden items-center gap-[clamp(1.25rem,3vw,2rem)] md:flex">
          ...
          <CTAButton
            href="/webdev-landing"
            event="cta-navbar"
            className="ml-4 inline-flex items-center gap-2 rounded-lg bg-[var(--color-blood)] px-4 py-[0.45rem] text-[clamp(0.65rem,0.9vw,0.75rem)] font-semibold text-[var(--color-text-light)] shadow transition-transform hover:scale-105 hover:bg-[var(--color-crimson)]"
          >
            Get Started →
          </CTAButton>
        </nav>
