...
export default function StickyHeader({ light = false }: HeaderProps) {
  ...
  const textColor = scrolled || !light ? 'text-silver' : 'text-charcoal';
      ...
      >
        ...
        <nav className="hidden items-center gap-[clamp(1.25rem,3vw,2rem)] md:flex">
          ...
          <CTAButton
            href="/webdev-landing"
            event="cta-navbar"
            className="ml-4 inline-flex items-center gap-2 rounded-lg bg-blood px-4 py-[0.45rem] text-[clamp(0.65rem,0.9vw,0.75rem)] font-semibold text-silver shadow transition-transform hover:scale-105 hover:brightness-110"
          >
            Get Started →
          </CTAButton>
        </nav>
