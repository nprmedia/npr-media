...
export default function ValuesCarousel() {
  ...
  return (
    <div className="relative mx-auto h-screen max-w-md overflow-hidden bg-[var(--color-antique)]">
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10" style={{ height: '33%' }}>
        <div className="h-full bg-gradient-to-b from-[var(--color-antique)] via-[var(--color-antique)]/80 to-transparent" />
      </div>
      <div
        className="pointer-events-none absolute inset-x-0 bottom-0 z-10"
        style={{ height: '33%' }}
      >
        <div className="h-full bg-gradient-to-t from-[var(--color-antique)] via-[var(--color-antique)]/80 to-transparent" />
      </div>
      <div
        ref={containerRef}
          ...
          return (
            <section key={slide.title} className="snap-center py-4">
              <motion.div
                ...
                style={{ opacity }}
                className="rounded-xl bg-[var(--color-antique)] p-6 text-[var(--color-charcoal)] shadow"
              >
                <p className="text-xl leading-snug font-bold">{slide.title}</p>
                <p className="mt-2 text-lg leading-relaxed text-[var(--color-sepia)]">{slide.description}</p>
              </motion.div>
            </section>
          );
        })}
