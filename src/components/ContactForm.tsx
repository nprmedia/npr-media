...
export default function ContactForm({ onSuccess }: ContactFormProps) {
  ...
  return (
    <motion.form
      ...
      className="space-y-6 rounded-2xl bg-sepia/80 p-6 shadow-xl ring-1 shadow-umber/10 ring-transparent backdrop-blur-md hover:ring-blood/20"
    >
      <motion.div
        className="relative"
      ...
      >
        ...
        />
        {placeholders.name && (
          <span className="pointer-events-none absolute top-1/2 left-10 -translate-y-1/2 text-sm text-olive">
            {placeholders.name}
          </span>
        )}
      </motion.div>
      ...
      <motion.button
        ...
        disabled={loading}
        className="relative flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-blood)] py-3 font-semibold text-[var(--color-text-light)] shadow-md ring-2 ring-blood/50 transition-transform hover:shadow-lg hover:bg-[var(--color-crimson)] disabled:opacity-60"
      >
        {loading ? (
        ...
        >
          ...
          <div className="pt-2">
            <a
              ...
              className="text-blood underline hover:text-crimson"
            >
              Book a Call
            </a>
          </div>
