        ...
        <div className="mx-auto mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {pricing.tiers.map((tier, index) => (
            <motion.div
              ...
              className={`relative flex flex-col overflow-hidden rounded-2xl border border-silver bg-sepia p-6 ${tier.highlight ? 'ring-2 ring-blood' : ''}`}
            >
              {tier.highlight && (
                <motion.div
                  className="pointer-events-none absolute inset-0 -z-10 rounded-2xl bg-blood/20"
