...
export default function TabbedPricing() {
  ...
  return (
    <section
      id="pricing"
      className="w-full border-t bg-[#1c1c1c] py-[clamp(5rem,10vw,8rem)] text-[#F2F3F4]"
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl space-y-4 text-center">
          <h2 className="text-[clamp(1.75rem,3.5vw,2.5rem)] font-bold tracking-tight text-[#F2F3F4]">
            ...
            {webDev.tiers.map((tier, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`relative rounded-2xl before:absolute before:inset-0 before:rounded-2xl ${isMiddleCard ? 'before:animate-pulse before:ring-2 before:ring-[#d4af37]' : ''}`}
                  >
                    <Card className="relative flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-gray-700 bg-gray-800 shadow-[0_0_0_1px_rgba(0,0,0,0.05),0_10px_15px_-3px_rgba(0,0,0,0.1),0_4px_6px_-4px_rgba(0,0,0,0.1)] backdrop-blur-lg transition-shadow hover:shadow-xl dark:hover:shadow-zinc-900/30">
                      <CardContent className="space-y-4 p-5">
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <h3 className="text-[clamp(1rem,1.8vw,1.25rem)] leading-tight font-semibold text-[#F2F3F4]">
                                {tier.title}
                              </h3>
                            </div>
                        ...
                        <ul className="scrollbar-thin scrollbar-thumb-gray-700 max-h-[180px] space-y-2 overflow-y-auto text-[clamp(0.8rem,1.2vw,0.9rem)] text-gray-300">
                          {tier.features.map((feature, i) => (
                            <li
                              key={i}
                              className={`flex items-start gap-2 ${i === 0 ? 'font-semibold text-[#F2F3F4]' : ''}`}
                            >
                              <span className="text-[#d4af37]">{i === 0 ? '✅' : '✓'}</span>{' '}
                              {feature}
                            </li>
                          ))}
                        </ul>
                        <div className="pt-2">
                          <button className="group mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-gray-600 bg-gray-800 px-[clamp(1rem,2.5vw,1.25rem)] py-[clamp(0.6rem,1.2vw,0.75rem)] text-[clamp(0.8rem,1vw,0.9rem)] font-medium text-[#F2F3F4] shadow-sm transition hover:scale-105 hover:bg-gray-700">
                            Start Plan
