  ...
  return (
    <Dialog.Root onOpenChange={() => setSubmitted(false)}>
      <Dialog.Trigger asChild>
        <button className="inline-flex items-center justify-center rounded-full bg-surface px-6 py-2 text-sm font-semibold text-foreground shadow hover:bg-muted">
          {triggerLabel}
        </button>
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-muted/80" />
        <Dialog.Content className="fixed top-1/2 left-1/2 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-surface p-6 text-foreground shadow-lg">
          ...
          <Dialog.Close className="absolute right-3 top-3 rounded p-1 text-foreground hover:bg-muted">
            <X className="h-4 w-4" />
          </Dialog.Close>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-3">
              ...
              <button type="submit" className="mt-2 w-full rounded bg-primary px-4 py-2 text-muted-foreground hover:bg-primary/90">
                Send Request
              </button>
            </form>
          )}
