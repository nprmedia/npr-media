  ...
  return (
    <Dialog.Root onOpenChange={() => setSubmitted(false)}>
      <Dialog.Trigger asChild>
        <button className="inline-flex items-center justify-center rounded-full bg-sepia px-6 py-2 text-sm font-semibold text-charcoal shadow hover:bg-sepia">
          {triggerLabel}
        </button>
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-antique/80" />
        <Dialog.Content className="fixed top-1/2 left-1/2 w-[90vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-sepia p-6 text-charcoal shadow-lg">
