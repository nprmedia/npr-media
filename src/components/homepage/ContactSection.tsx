  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-2xl text-center space-y-6">
        ...
        <p className="text-[clamp(0.9rem,1.6vw,1.125rem)] text-foreground">{subtitle}</p>
        <div className="pt-4">
          <Link
            href={cta.href}
        ...
        <p className="text-sm text-foreground">{trust}</p>
        {urgency && <p className="text-xs font-medium text-accent">{urgency}</p>}
        <div className="pt-2">
          <Link
            href={exitLink.href}
            className="text-[clamp(0.75rem,1vw,0.875rem)] text-foreground underline-offset-4 hover:underline"
          >
            {exitLink.label}
          </Link>
        </div>
