  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-2xl text-center space-y-6">
        ...
        <p className="text-[clamp(0.9rem,1.6vw,1.125rem)] text-[var(--color-antique)]">{subtitle}</p>
        <div className="pt-4">
          <Link
            href={cta.href}
            data-event="cta-final"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--color-blood)] px-[clamp(1.25rem,3vw,1.5rem)] py-[clamp(0.6rem,1.2vw,0.75rem)] text-[clamp(0.8rem,1vw,0.9rem)] font-semibold text-[var(--color-silver)] shadow-lg transition hover:scale-105 hover:bg-[var(--color-crimson)] hover:shadow-xl"
          >
            {cta.label}
            <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
        <p className="text-sm text-[var(--color-sepia)]">{trust}</p>
        {urgency && <p className="text-xs font-medium text-[var(--color-accent)]">{urgency}</p>}
        <div className="pt-2">
          <Link
            href={exitLink.href}
            className="text-[clamp(0.75rem,1vw,0.875rem)] text-[var(--color-sepia)] underline-offset-4 hover:underline"
          >
            {exitLink.label}
          </Link>
        </div>
