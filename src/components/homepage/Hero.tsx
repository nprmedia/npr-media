...
const HeroSection: React.FC<HeroProps> = ({ headline, subheadline, ctaText, ctaLink, image }) => {
  ...
  return (
    <section
      ...
      className="relative min-h-screen pb-[5vh] flex items-center justify-center bg-background font-sans overflow-hidden"
    >
      <div
        ref={containerRef}
        ...
        <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="ml-[15vw] w-full">
          ...
          {subheadline && (
            <motion.p
              variants={textVariants}
              custom={1.5}
              className="text-foreground mb-4 max-w-xl text-[clamp(0.75rem,1.6vw,1rem)] hover:scale-102"
            >
              {subheadline}
            </motion.p>
          )}
          {ctaText && ctaLink && (
            ...
            >
              ...
              <Link
                ...
                href={ctaLink}
                data-event="cta-hero"
                className="inline-flex items-center justify-center rounded-full px-4 py-[0.4rem] text-[clamp(0.7rem,1vw,0.9rem)] font-semibold text-muted-foreground shadow-lg ring-1 bg-primary transition hover:bg-primary/90"
              >
                {ctaText}
              </Link>
      ...
      {isStickyVisible && (
        <div className="fixed bottom-36 left-1/2 z-50 -translate-x-1/2 rounded-full bg-primary px-4 py-2 text-sm font-bold text-muted-foreground opacity-90 shadow-xl hover:scale-105 hover:bg-primary/90">
