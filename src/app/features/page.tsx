...
function Hero() {
  return (
    <section className="mx-auto max-w-6xl space-y-6 px-4 py-[clamp(5rem,10vw,8rem)] text-center">
      <motion.h1
        variants={fadeIn}
        initial="hidden"
        animate="show"
        ...
        <ul className="grid gap-6 md:grid-cols-3">
          {betterThanAI.map((item, idx) => (
            ...
            >
              ...
              <p className="text-[clamp(0.8rem,1.2vw,0.9rem)] text-gray-700">
                {item.body}
              </p>
            </motion.li>
          ))}
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {features.map((f, idx) => {
            ...
            >
              ...
              <h3 className="text-[clamp(1.25rem,2.5vw,1.75rem)] font-semibold">
                {f.title}
              </h3>
              <p className="text-[clamp(0.8rem,1.2vw,0.9rem)] text-gray-300">
                {f.body}
              </p>
            </motion.div>
            )
        ...
        <ol className="grid gap-8 md:grid-cols-3">
          {steps.map((s, idx) => {
            ...
            return (
              ...
              >
                {Icon && <Icon className="mx-auto h-8 w-8" aria-hidden="true" />}
                <div className="text-[clamp(1.1rem,2vw,1.5rem)] font-semibold">
                  {s.title}
                </div>
                <p className="text-[clamp(0.8rem,1.2vw,0.9rem)] text-gray-700">
                  {s.body}
    ...
    >
      <div className="container mx-auto space-y-6 px-4">
        <h2 className="text-[clamp(1.5rem,3vw,2rem)] font-bold">
          Ready for Your Next-Level Site?
        </h2>
